...
global.reloadHandler = function () {
  ...
  conn.welcome = '*SEJA-BEMVINDO* *@user 𝚂𝙴𝙹𝙰 𝙱𝙴𝙼 𝚅𝙸𝙽𝙳𝙾 𝙰𝙾 𝙼𝙴𝙻𝙷𝙾𝚁 𝙶𝚁𝚄𝙿𝙾 @subject 𝙼𝙴𝚄 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 /𝙼𝙴𝙽𝚄*'
  conn.bye = '*TCHAU!*  *@user 𝚅𝙰𝙸 𝙿𝙴𝙻𝙰 𝚂𝙾𝙼𝙱𝚁𝙰 𝙵𝚄𝙲𝙺!!!*'
...
async function _quickTest() {
  ...
  if (!s.convert && !s.magick && !s.gm) conn.logger.warn('Os adesivos podem não funcionar sem o imagemagick se o libwebp do ffmpeg não estiver instalado (pacote install imagemagick)')
}
...
if (!global.support.ffmpeg) conn.logger.warn('Por favor instale o ffmpeg para enviar vídeos (pacote install ffmpeg)')
if (!global.support.ffmpegWebp) conn.logger.warn('Os adesivos não podem ser animados sem libwebp no ffmpeg (--emable-ibwebp durante a compilação do ffmpeg)')
if (!global.support.convert) conn.logger.warn('Os adesivos podem não funcionar sem o imagemagick se o libwebp do ffmpeg não estiver instalado (pacote install imagemagick)')
