...
let handler = async (m, { conn, text, participants }) => {
  ...
  for (let user of response.participants.filter(user => Object.values(user)[0].code == 403)) {
    ...
    let teks = `*Convidativo* @${jid.split('@')[0]} *usando convite*... `
    m.reply(teks, null, {
      contextInfo: {
        mentionedJid: conn.parseMention(teks)
      }
    })
    await conn.sendGroupV4Invite(m.chat, jid, invite_code, invite_code_exp, false, '*Convite para aderir ao meu grupo WhatsApp*', jpegThumbnail ? {
      jpegThumbnail
    } : {})
  }
}
