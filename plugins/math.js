...
Exemplos de uso: ${usedPrefix}math medium
`.trim(), m)
...
`.trim(), m)
  let id = m.chat
  if (id in global.math) return conn.reply(m.chat, 'Ainda há perguntas sem resposta neste bate-papo', global.math[id][0])
  let math = genMath(mode)
  global.math[id] = [
    await conn.reply(m.chat, `Quantos resultados de *${math.str}*?\n\nTempo esgotado: ${(math.time / 1000).toFixed(2)} detik\nBônus de resposta correta: ${math.bonus} XP`, m),
    math, 4,
    setTimeout(() => {
...
let modes = {
  noob: [-3, 3,-3, 3, '+-', 20000, 100],
  easy: [-10, 10, -10, 10, '*/+-', 30000, 300],
  medium: [-40, 40, -20, 20, '*/+-', 40000, 800],
  hard: [-100, 100, -70, 70, '*/+-', 60000, 1200],
